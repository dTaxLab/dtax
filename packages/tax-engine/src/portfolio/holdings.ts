...
export function analyzeHoldings(
  ...
  const lotHoldings: LotHolding[] = activeLots.map((lot) => {
    ...
    const isLongTerm =
      asOfDate.getTime() - lot.acquiredAt.getTime() > ONE_YEAR_MS;
  ...
  for (const [asset, holdings] of byAsset) {
    ...
    const position: AssetPosition = {
      ...
      earliestAcquired: new Date(
        Math.min(...holdings.map((h) => h.acquiredAt.getTime())),
      ),
  ...
  positions.sort(
    (a, b) =>
      (b.currentValueUsd ?? b.totalCostBasis) -
      (a.currentValueUsd ?? a.totalCostBasis),
  );
  ...
  if (currentPrices && currentPrices.size > 0) {
    for (const pos of positions) {
      ...
      const unrealizedLoss = loseLots.reduce(
        (s, l) => s + (l.unrealizedGainLoss ?? 0),
        0,
      );
      const currentValue = loseLots.reduce(
        (s, l) => s + (l.currentValueUsd ?? 0),
        0,
      );
  ...
  const totalUnrealizedGainLoss =
    hasPrices && totalCurrentValue !== undefined
