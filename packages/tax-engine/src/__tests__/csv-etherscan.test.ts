...
describe("detectCsvFormat", () => {
  it("should detect etherscan format", () => {
    const csv =
      '"Txhash","Blockno","UnixTimestamp","DateTime (UTC)","From","To","ContractAddress","Value_IN(ETH)","Value_OUT(ETH)"\n';
...
describe("parseEtherscanCsv", () => {
  ...
  it("should support custom native asset", () => {
    const csv =
      '"Txhash","Blockno","UnixTimestamp","DateTime (UTC)","From","To","ContractAddress","Value_IN(MATIC)","Value_OUT(MATIC)","CurrentValue","TxnFee(MATIC)","TxnFee(USD)","Historical $Price/Matic","Status","ErrCode"\n' +
