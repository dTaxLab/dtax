...
describe('isOkxCsv', () => {
    it('detects OKX format by header', () => {
        const csv = 'Order ID,Trade ID,Trade Time,Pair,Side,Price,Amount,Total,Fee,Fee Currency\n';
        expect(isOkxCsv(csv)).toBe(true);
    });

    it('detects OKX format with instrument column', () => {
        const csv = 'Order ID,Trade ID,Trade Time,Instrument,Side,Price,Amount,Total,Fee,Fee Currency\n';
        expect(isOkxCsv(csv)).toBe(true);
    });

    it('rejects non-OKX format', () => {
        expect(isOkxCsv('Timestamp,Transaction Type,Asset,Quantity Transacted')).toBe(false);
    });
});
...
describe('parseOkxCsv', () => {
    const HEADER = 'Order ID,Trade ID,Trade Time,Pair,Side,Price,Amount,Total,Fee,Fee Currency';
