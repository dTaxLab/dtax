...
describe("1099-DA Parser", () => {
  ...
  it("should normalize asset names", () => {
    ...
    expect(result.entries.map((e) => e.asset)).toEqual([
      "BTC",
      "ETH",
      "SOL",
      "DOGE",
  ...
  it("should handle entries without cost basis (pre-2027)", () => {
    const csv = ["asset,date_sold,gross_proceeds", "BTC,2025-03-15,50000"].join(
      "\n",
    );
...
describe("Reconciliation Engine", () => {
  function makeBrokerEntry(
    overrides: Partial<Form1099DAEntry> = {},
  ): Form1099DAEntry {
    return {
      rowIndex: 1,
      asset: "BTC",
  ...
  function makeDtaxEntry(
    overrides: Partial<DtaxDisposition> = {},
  ): DtaxDisposition {
    return {
      eventId: "evt-1",
      asset: "BTC",
  ...
  it("should identify missing entries in DTax", () => {
    const report = reconcile(
      [
        makeBrokerEntry(),
        makeBrokerEntry({ asset: "ETH", grossProceeds: 3000 }),
      ],
      [makeDtaxEntry()],
      { taxYear: 2025, brokerName: "Coinbase" },
    );
  ...
  it("should identify missing entries in 1099-DA", () => {
    const report = reconcile(
      ...
      [
        makeDtaxEntry(),
        makeDtaxEntry({ eventId: "evt-2", asset: "SOL", proceeds: 200 }),
      ],
      { taxYear: 2025, brokerName: "Coinbase" },
    );
