...
describe("isGateCsv", () => {
  it("detects Gate.io format by header (standard)", () => {
    const csv =
      "No,Pair,Side,Role,Filled Price,Filled Amount,Total,Fee,Fee Currency,Date\n";
    expect(isGateCsv(csv)).toBe(true);
  });

  it("detects Gate.io format by header (API-style)", () => {
    const csv =
      "id,currency_pair,side,role,amount,price,fee,fee_currency,create_time,point_fee\n";
    expect(isGateCsv(csv)).toBe(true);
  });

  it("rejects non-Gate.io format", () => {
    expect(
      isGateCsv("Timestamp,Transaction Type,Asset,Quantity Transacted"),
    ).toBe(false);
  });
});
...
describe("parseGateCsv", () => {
  ...
  it("computes total from price * amount when total is missing", () => {
    const header =
      "No,Pair,Side,Role,Filled Price,Filled Amount,Fee,Fee Currency,Date";
