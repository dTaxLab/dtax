...
describe("isGeminiCsv", () => {
  it("detects Gemini format by header", () => {
    const csv =
      "Date,Time (UTC),Type,Symbol,Specification,Liquidity Indicator,Trading Fee Rate (bps),USD Amount ($),Trading Fee (USD) ($),USD Balance ($),BTC Amount (BTC),Trading Fee (BTC) (BTC),BTC Balance (BTC)\n";
    expect(isGeminiCsv(csv)).toBe(true);
  });

  it("rejects non-Gemini format", () => {
    expect(
      isGeminiCsv("Timestamp,Transaction Type,Asset,Quantity Transacted"),
    ).toBe(false);
  });
});
...
describe("detectCsvFormat", () => {
  it("auto-detects Gemini", () => {
    const csv =
      "Date,Time (UTC),Type,Symbol,Specification,Liquidity Indicator,Trading Fee Rate (bps),USD Amount ($)\n";
...
2025-01-15,10:30:00,Buy,BTCUSD,Limit,Maker,25,-50000.00,12.50,10000.00,1.00000000,,11.00000000`;
  ...
  it("handles ETH trading pair", () => {
    const header =
      "Date,Time (UTC),Type,Symbol,Specification,Liquidity Indicator,Trading Fee Rate (bps),USD Amount ($),Trading Fee (USD) ($),USD Balance ($),ETH Amount (ETH),Trading Fee (ETH) (ETH),ETH Balance (ETH)";
    const csv = `${header}
