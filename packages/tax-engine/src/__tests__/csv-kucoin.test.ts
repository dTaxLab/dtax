...
describe('isKuCoinCsv', () => {
    it('detects KuCoin format by header', () => {
        const csv = 'trade_id,symbol,order_type,deal_price,amount,direction,funds,fee_currency,fee,created_at\n';
        expect(isKuCoinCsv(csv)).toBe(true);
    });

    it('rejects non-KuCoin format', () => {
        expect(isKuCoinCsv('Timestamp,Transaction Type,Asset,Quantity Transacted')).toBe(false);
    });
});
...
describe('parseKuCoinCsv', () => {
    const HEADER = 'trade_id,symbol,order_type,deal_price,amount,direction,funds,fee_currency,fee,created_at';
    ...
    it('handles date string fallback', () => {
        const header = 'trade_id,symbol,order_type,deal_price,amount,direction,funds,fee_currency,fee,created_date';
