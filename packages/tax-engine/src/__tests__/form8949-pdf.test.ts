...
function makeResult(overrides: Partial<CalculationResult> = {}): CalculationResult {
    return {
        event: { id: 'e1', asset: 'BTC', amount: 1, proceedsUsd: 50000, date: new Date('2025-06-15'), sourceId: 's1' },
...
describe('generateForm8949Pdf', () => {
    ...
    it('handles multiple boxes', async () => {
        ...
        const shortTerm = makeResult({
            event: { id: 'e2', asset: 'ETH', amount: 5, proceedsUsd: 10000, date: new Date('2025-06-15'), sourceId: 's1' },
        ...
        const report = generateForm8949([longTerm, shortTerm], { taxYear: 2025, lotDates, reportingBasis: 'none' });
    ...
    it('handles empty report', async () => {
        const report = generateForm8949([], { taxYear: 2025, lotDates: new Map(), reportingBasis: 'none' });
