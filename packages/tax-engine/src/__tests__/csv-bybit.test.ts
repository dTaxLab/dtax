...
describe('isBybitCsv', () => {
    it('detects Bybit format by header (standard)', () => {
        const csv = 'Order No,Symbol,Side,Avg. Filled Price,Filled Qty,Total,Fee,Fee Currency,Order Time,Order Type\n';
        expect(isBybitCsv(csv)).toBe(true);
    });

    it('detects Bybit format by header (API-style)', () => {
        const csv = 'orderId,symbol,side,execPrice,execQty,execValue,execFee,feeCurrency,execTime\n';
        expect(isBybitCsv(csv)).toBe(true);
    });

    it('rejects non-Bybit format', () => {
        expect(isBybitCsv('Timestamp,Transaction Type,Asset,Quantity Transacted')).toBe(false);
    });
});
...
describe('parseBybitCsv', () => {
    const HEADER = 'Order No,Symbol,Side,Avg. Filled Price,Filled Qty,Total,Fee,Fee Currency,Order Time,Order Type';
    ...
    it('computes total from price * qty when total is missing', () => {
        const header = 'Order No,Symbol,Side,Avg. Filled Price,Filled Qty,Fee,Fee Currency,Order Time';
