...
export function isBitgetCsv(csv: string): boolean {
    ...
    if (firstLine.includes('trading pair') && firstLine.includes('side') &&
        (firstLine.includes('filled') || firstLine.includes('total'))) return true;
    if (firstLine.includes('priceavg') && firstLine.includes('basevolume')) return true;
    if (firstLine.includes('symbol') && firstLine.includes('side') &&
        firstLine.includes('quotevolume')) return true;
    return false;
}
...
function parseTradeRow(
    ...
    const tsRaw = row['order time'] || row['trade time'] || row['ctime'] ||
    ...
    // Parse symbol
    const symbolRaw = (row['trading pair'] || row['symbol'] || row['pair'] || '').trim();
    ...
    const price = safeParseNumber(row['filled price'] || row['priceavg'] || row['price avg'] ||
        row['price'] || row['avg. filled price']);
    const qty = safeParseNumber(row['filled amount'] || row['size'] || row['basevolume'] ||
        row['base volume'] || row['qty'] || row['amount']);
    const total = safeParseNumber(row['total'] || row['quotevolume'] || row['quote volume'] ||
        row['value'] || row['funds']);
