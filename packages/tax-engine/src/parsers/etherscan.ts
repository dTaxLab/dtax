...
export function isEtherscanCsv(csv: string): boolean {
    ...
    return (firstLine.includes('txhash') || firstLine.includes('"txhash"')) &&
...
export function isEtherscanErc20Csv(csv: string): boolean {
    const firstLine = csv.split('\n')[0]?.toLowerCase() || '';
    return (firstLine.includes('txhash') || firstLine.includes('"txhash"')) &&
        (firstLine.includes('tokensymbol') || firstLine.includes('token symbol') ||
...
export function parseEtherscanCsv(
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        try {
            ...
            const valueIn = safeParseNumber(row[`value_in(${nativeAsset.toLowerCase()})`] || row['value_in(eth)']);
            const valueOut = safeParseNumber(row[`value_out(${nativeAsset.toLowerCase()})`] || row['value_out(eth)']);
            const txnFeeEth = safeParseNumber(row[`txnfee(${nativeAsset.toLowerCase()})`] || row['txnfee(eth)']);
            const txnFeeUsd = safeParseNumber(row['txnfee(usd)']);
            const histPrice = safeParseNumber(row[`historical $price/${nativeAsset.toLowerCase()}`] || row['historical $price/eth']);
            ...
            if (!hasValueIn && !hasValueOut) {
                if (fromIsUser && txnFeeEth && txnFeeEth > 0) {
                    transactions.push({
                        ...
                        feeAmount: txnFeeEth,
                        feeValueUsd: txnFeeUsd || (histPrice ? txnFeeEth * histPrice : undefined),
                    });
                }
                continue;
            }
            ...
            if (fromIsUser && txnFeeEth && txnFeeEth > 0) {
                ...
                tx.feeValueUsd = txnFeeUsd || (histPrice ? txnFeeEth * histPrice : undefined);
            }
...
export function parseEtherscanErc20Csv(
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        try {
            ...
            const from = (row['from'] || '').toLowerCase();
            const to = (row['to'] || '').toLowerCase();
            const symbol = (row['tokensymbol'] || row['token symbol'] || '').toUpperCase();
            const value = safeParseNumber(row['value']);
            const decimals = parseInt(row['tokendecimal'] || row['token decimal'] || '18');
            const txhash = row['txhash'] || '';
