...
function mapCryptoComType(kind: string, description: string): ParsedTransaction['type'] {
    ...
    if (k === 'crypto_transfer') return d.includes('sent') ? 'TRANSFER_OUT' : 'TRANSFER_IN';
    if (k === 'crypto_exchange' || k === 'exchange' || k === 'swap') return 'TRADE';
    if (k === 'rewards_platform_deposit_credited' || k === 'reward' || k === 'staking_reward') return 'STAKING_REWARD';
    if (k === 'mco_stake_reward' || k === 'stake_reward') return 'STAKING_REWARD';
    if (k === 'crypto_earn_interest_paid' || k === 'interest') return 'INTEREST';
    if (k === 'referral_card_cashback' || k === 'reimbursement' || k === 'cashback') return 'INTEREST';
    if (k === 'airdrop' || k === 'admin_wallet_credited') return 'AIRDROP';
    if (k === 'dust_conversion_credited' || k === 'dust_conversion_debited') return 'TRADE';
...
export function isCryptoComCsv(csv: string): boolean {
    ...
    // Newer format
    if (firstLine.includes('sent amount') && firstLine.includes('received amount') && firstLine.includes('fee currency')) {
        return true;
    }
    // App format
    if (firstLine.includes('transaction kind') && firstLine.includes('native amount')) {
        return true;
    }
    return false;
}
...
export function parseCryptoComCsv(csv: string): CsvParseResult {
    ...
    if (objects.length === 0) {
        return { transactions, errors, summary: { totalRows: 0, parsed: 0, failed: 0, format: 'crypto_com' } };
    }
