...
function mapKrakenType(type: string, subtype: string, amount: number): ParsedTransaction['type'] {
...
export function isKrakenCsv(csv: string): boolean {
    ...
    return firstLine.includes('txid') &&
...
export function parseKrakenCsv(csv: string): CsvParseResult {
    ...
    const byRefId = new Map<string, { rows: typeof objects; indices: number[] }>();
    ...
    for (const [, group] of byRefId) {
        ...
        try {
            ...
            if (group.rows.length === 2) {
                ...
                const receivedAmount = Math.abs(safeParseNumber(received['amount']) || 0);
                ...
                if (totalFee > 0) {
                    ...
                    tx.feeAsset = normalizeKrakenAsset(
                        (safeParseNumber(a['fee']) || 0) > 0 ? (a['asset'] || '') : (b['asset'] || '')
                    );
                }
