  ...
  for (let i = 0; i < objects.length; i++) {
    ...
    try {
      ...
      if (!tsRaw) {
        errors.push({
          row: rowNum,
          message: `Missing timestamp (column: "${map.timestamp}")`,
        });
        continue;
      }
      ...
      const type = map.type
        ? normalizeType(row[map.type] || "UNKNOWN")
        : "UNKNOWN";

      const tx: ParsedTransaction = {
        type,
        timestamp,
        receivedAsset: map.receivedAsset
          ? row[map.receivedAsset] || undefined
          : undefined,
        receivedAmount: map.receivedAmount
          ? safeParseNumber(row[map.receivedAmount])
          : undefined,
        receivedValueUsd: map.receivedValueUsd
          ? safeParseNumber(row[map.receivedValueUsd])
          : undefined,
        sentAsset: map.sentAsset ? row[map.sentAsset] || undefined : undefined,
        sentAmount: map.sentAmount
          ? safeParseNumber(row[map.sentAmount])
          : undefined,
        sentValueUsd: map.sentValueUsd
          ? safeParseNumber(row[map.sentValueUsd])
          : undefined,
        feeAsset: map.feeAsset ? row[map.feeAsset] || undefined : undefined,
        feeAmount: map.feeAmount
          ? safeParseNumber(row[map.feeAmount])
          : undefined,
        feeValueUsd: map.feeValueUsd
      ...
      if (!tx.receivedAsset && !tx.sentAsset) {
        errors.push({
          row: rowNum,
          message: "No asset found (need received or sent asset)",
        });
        continue;
      }
