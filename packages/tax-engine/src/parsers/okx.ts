...
export function isOkxCsv(csv: string): boolean {
    ...
    return (firstLine.includes('pair') || firstLine.includes('instrument')) &&
...
function parseTradeRow(
    ...
    // Parse timestamp
    const tsRaw = row['trade time'] || row['tradetime'] || row['time'] || row['timestamp'] || '';
    ...
    const pair = (row['pair'] || row['instrument'] || row['symbol'] || '').toUpperCase().trim();
    ...
    const amount = safeParseNumber(row['amount'] || row['quantity'] || row['filled qty']);
    const total = safeParseNumber(row['total'] || row['funds'] || row['filled amount']);
