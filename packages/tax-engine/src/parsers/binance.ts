...
export function isBinanceCsv(csv: string): boolean {
    ...
    return (firstLine.includes('date(utc)') || firstLine.includes('date (utc)')) &&
...
export function parseBinanceCsv(csv: string): CsvParseResult {
    ...
    for (let i = 0; i < objects.length; i++) {
        ...
        try {
            ...
            if (!pair) {
                errors.push({ row: rowNum, message: `Cannot parse pair: "${pairStr}"` });
                continue;
            }
            ...
            const executed = safeParseNumber(row['executed'] || row['filled'] || row['amount']);
            const price = safeParseNumber(row['price'] || row['avg trading price']);
            const total = safeParseNumber(row['amount'] || row['total'] || row['vol']);
            ...
            if (type === 'BUY') {
                ...
                tx.receivedValueUsd = total || (executed && price ? executed * price : undefined);
                tx.sentAsset = pair.quote;
                tx.sentAmount = total;
            } else {
                tx.sentAsset = pair.base;
                tx.sentAmount = executed;
                tx.sentValueUsd = total || (executed && price ? executed * price : undefined);
