...
const FIAT_CURRENCIES = new Set(['USD', 'USDT', 'USDC', 'EUR', 'GBP', 'AUD', 'CAD', 'JPY']);
...
export function isKuCoinCsv(csv: string): boolean {
    ...
    return (firstLine.includes('symbol') || firstLine.includes('trade_id')) &&
...
function parseTradeRow(
    ...
    const createdDate = row['created_date'] || row['createddate'] || row['time'] || '';
    ...
    if (!timestamp) {
        errors.push({ row: rowNum, message: `Invalid date: "${createdAt || createdDate}"` });
        return null;
    }
    ...
    const direction = (row['direction'] || row['side'] || '').toLowerCase().trim();
    const amount = safeParseNumber(row['amount']) || safeParseNumber(row['quantity']);
    const funds = safeParseNumber(row['funds']) || safeParseNumber(row['total']);
    const fee = safeParseNumber(row['fee']);
    const feeCurrency = (row['fee_currency'] || row['feecurrency'] || '').toUpperCase().trim();
