...
export function parseGeminiCsv(csv: string): CsvParseResult {
  ...
  if (objects.length === 0) {
    return {
      transactions,
      errors,
      summary: { totalRows: 0, parsed: 0, failed: 0, format: "gemini" },
    };
  }
...
function parseTradeRow(
  ...
  if (amountsByCurrency.length < 2) {
    errors.push({
      row: rowNum,
      message: `Cannot find trade pair amounts for symbol "${symbol}"`,
    });
    return null;
  }
  ...
  if (!received || !sent) {
    errors.push({
      row: rowNum,
      message: `Cannot determine trade direction for "${symbol}"`,
    });
    return null;
  }
