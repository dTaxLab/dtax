...
export function isGateCsv(csv: string): boolean {
    ...
    if (firstLine.includes('currency_pair') && firstLine.includes('side')) return true;
    if (firstLine.includes('pair') && firstLine.includes('role') &&
        (firstLine.includes('filled price') || firstLine.includes('filled amount'))) return true;
    return false;
}
...
function parseTradeRow(
    ...
    // Parse timestamp
    const tsRaw = row['date'] || row['create_time'] || row['time'] || row['timestamp'] || '';
    ...
    const pairRaw = (row['pair'] || row['currency_pair'] || row['symbol'] || '').toUpperCase().trim();
    ...
    const amount = safeParseNumber(row['filled amount'] || row['amount'] || row['qty'] || row['quantity']);
    const price = safeParseNumber(row['filled price'] || row['price'] || row['avg. filled price']);
    const total = safeParseNumber(row['total'] || row['funds'] || row['value']);
    const fee = safeParseNumber(row['fee']);
    const feeCurrency = (row['fee currency'] || row['fee_currency'] || row['feecurrency'] || '').toUpperCase().trim();
