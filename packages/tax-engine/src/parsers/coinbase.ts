...
export function parseCoinbaseCsv(csv: string): CsvParseResult {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    try {
      ...
      const asset =
        colIdx.asset >= 0 ? row[colIdx.asset]?.toUpperCase() : undefined;
      const quantity =
        colIdx.quantity >= 0
          ? safeParseNumber(row[colIdx.quantity])
          : undefined;
      const total =
        colIdx.total >= 0 ? safeParseNumber(row[colIdx.total]) : undefined;
      const fees =
        colIdx.fees >= 0 ? safeParseNumber(row[colIdx.fees]) : undefined;
