...
function parseDate(raw: string | undefined): Date | undefined {
    if (!raw || raw.trim() === '' || raw.trim().toLowerCase() === 'various') return undefined;
...
function detectFormat(headers: string[]): 'coinbase' | 'generic' {
    ...
    if (joined.includes('asset name') && joined.includes('gross proceeds')) return 'coinbase';
    return 'generic';
}
...
export function parse1099DA(
    ...
    if (rows.length === 0) {
        return { entries, errors: [{ row: 0, message: 'No data rows found' }], brokerName, taxYear };
    }
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        try {
            ...
            const getVal = (key: string): string | undefined => {
                ...
                return row[colName] ?? Object.entries(row).find(
                    ([k]) => k.toLowerCase().replace(/[_\s]/g, '') === colName.toLowerCase().replace(/[_\s]/g, '')
                )?.[1];
            };
            ...
            if (!asset || !dateSoldRaw || !grossProceedsRaw) {
                errors.push({ row: rowNum, message: 'Missing required fields (asset, date_sold, gross_proceeds)' });
                continue;
            }
            ...
            if (grossProceeds === undefined) {
                errors.push({ row: rowNum, message: `Invalid proceeds: ${grossProceedsRaw}` });
                continue;
            }
            ...
            errors.push({ row: rowNum, message: `Parse error: ${e instanceof Error ? e.message : 'unknown'}` });
        }
    }
