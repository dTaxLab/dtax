...
function determineStatus(
    ...
    const proceedsMatch = Math.abs(broker.grossProceeds - dtax.proceeds) <= PROCEEDS_TOLERANCE;
    const basisMatch = broker.costBasis === undefined
        || Math.abs(broker.costBasis - dtax.costBasis) <= PROCEEDS_TOLERANCE;

    if (proceedsMatch && basisMatch) return 'matched';
    if (!proceedsMatch && !basisMatch && broker.costBasis !== undefined) return 'both_mismatch';
...
function generateRebuttal(item: ReconciliationItem): string | undefined {
    switch (item.status) {
        ...
        case 'proceeds_mismatch':
            return `Proceeds differ by $${Math.abs(item.proceedsDiff).toFixed(2)}. ` +
                `Verify transaction timing and exchange rate used. The broker may have used a different valuation timestamp.`;
        case 'basis_mismatch':
            return `Cost basis differs by $${Math.abs(item.costBasisDiff).toFixed(2)}. ` +
        ...
        case 'both_mismatch':
            return `Both proceeds ($${Math.abs(item.proceedsDiff).toFixed(2)}) and cost basis ($${Math.abs(item.costBasisDiff).toFixed(2)}) differ. ` +
                `Review the full transaction history. The broker may be using a different cost basis method.`;
        case 'missing_in_dtax':
            return `Transaction reported by broker but not found in your records. ` +
                `Check if this was imported correctly or if it was an overlooked transaction.`;
        case 'missing_in_1099da':
            return `Transaction in your records but not on 1099-DA. ` +
        ...
        case 'internal_transfer_misclassified':
            return `The broker reported this as a taxable disposition, but DTax classified it as an internal transfer. ` +
...
export function reconcile(
    ...
    for (let bi = 0; bi < brokerEntries.length; bi++) {
        ...
        for (let di = 0; di < dtaxDispositions.length; di++) {
            ...
            if (broker.asset === dtax.asset
                ...
                && withinProceedsRange(broker.grossProceeds, dtax.proceeds, FUZZY_PROCEEDS_RATIO)) {

                const score = Math.abs(broker.grossProceeds - dtax.proceeds)
                    + Math.abs(broker.dateSold.getTime() - dtax.dateSold.getTime()) / (24 * 60 * 60 * 1000);
    ...
    const summary = {
        ...
        proceedsMismatch: items.filter(i => i.status === 'proceeds_mismatch').length,
        ...
        missingIn1099da: items.filter(i => i.status === 'missing_in_1099da').length,
        internalTransferMisclassified: items.filter(i => i.status === 'internal_transfer_misclassified').length,
        netProceedsDiff: Math.round(items.reduce((s, i) => s + i.proceedsDiff, 0) * 100) / 100,
        netGainLossDiff: Math.round(items.reduce((s, i) => s + i.gainLossDiff, 0) * 100) / 100,
    };
