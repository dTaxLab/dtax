...
export async function generateReconcilePdf(
  ...
  return new Promise((resolve, reject) => {
    ...
    // ── Cover / Title ─────────────────────────────────
    // CPA firm logo (top-left corner, first page only)
    if (options.firmLogoBuffer) {
      renderFirmLogo(doc, MARGIN, options.firmLogoBuffer);
    }
    doc
      .fontSize(16)
      .font("Helvetica-Bold")
      .text(
        ...
        MARGIN,
        options.firmLogoBuffer ? MARGIN + 64 : MARGIN,
        { width: CONTENT_WIDTH },
      );
    ...
    const summaryRows: [string, string][] = [
      ...
      [
        "Transfer misclassified",
        String(report.summary.internalTransferMisclassified),
      ],
    ...
    doc.text("Broker Proceeds", COL.broker, headerY, {
      width: 85,
      align: "right",
    });
    ...
    for (const item of nonMatchedFirst) {
      ...
      doc.text(
        fmtDate(dateSold instanceof Date ? dateSold.toISOString() : dateSold),
        COL.date,
        rowY,
        { width: 65 },
      );
      ...
      doc
        .fillColor(item.riskLevel === "INFO" ? "#374151" : "#111827")
        .text(STATUS_LABEL[item.status] ?? item.status, COL.status, rowY, {
          width: 105,
        });
      doc
        .fillColor(RISK_COLORS[item.riskLevel] ?? "#6b7280")
        .text(item.riskLevel, COL.risk, rowY, { width: 55 });
      doc.fillColor("#111827");
      doc.text(
        brokerProceeds != null ? fmtUsd(brokerProceeds) : "—",
        COL.broker,
        rowY,
        { width: 85, align: "right" },
      );
      doc.text(
        dtaxProceeds != null ? fmtUsd(dtaxProceeds) : "—",
        COL.dtax,
        rowY,
        { width: 85, align: "right" },
      );
      const diffColor =
        item.proceedsDiff === 0
          ? "#6b7280"
          : Math.abs(item.proceedsDiff) > 0
            ? "#dc2626"
            : "#16a34a";
      doc
        .fillColor(diffColor)
        .text(
          item.proceedsDiff !== 0 ? fmtUsd(item.proceedsDiff) : "—",
          COL.diff,
          rowY,
    ...
    doc.moveDown(1);
    const footerText = options.preparedBy
      ? `Prepared by ${options.preparedBy} · Powered by dTax (getdtax.com) — For informational purposes only`
