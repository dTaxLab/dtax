...
function getBoxSummary(
    ...
    box: string,
): { proceeds: number; costBasis: number; adjustments: number; gainLoss: number } {
    const summary = boxSummaries.find(s => s.box === box);
    if (!summary) return { proceeds: 0, costBasis: 0, adjustments: 0, gainLoss: 0 };
    return {
...
export function generateScheduleD(
    ...
    const partI: ScheduleDLine[] = [
        ...
        {
            lineNumber: '2',
            description: 'Short-term from Form 8949, Box B (basis NOT reported to IRS)',
            ...boxB,
        },
        {
            lineNumber: '3',
    ...
    const partII: ScheduleDLine[] = [
        ...
        {
            lineNumber: '9',
            description: 'Long-term from Form 8949, Box E (basis NOT reported to IRS)',
            ...boxE,
        },
        {
            lineNumber: '10',
    ...
    if (combinedNetGainLoss < 0) {
        ...
        carryoverLoss = round2(Math.abs(combinedNetGainLoss) - capitalLossDeduction);
    }
