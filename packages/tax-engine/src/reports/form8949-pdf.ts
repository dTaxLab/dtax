...
import type { Form8949Report, Form8949Line, Form8949BoxSummary } from './form8949';
...
function renderForm8949Page(doc: PDFKit.PDFDocument, opts: PageOptions): void {
    ...
    doc.fontSize(10).font('Helvetica')
        .text('Sales and Other Dispositions of Capital Assets', MARGIN, y, { align: 'center', width: CONTENT_WIDTH });
    y += 14;

    doc.fontSize(8).font('Helvetica')
        .text(`Tax Year ${opts.taxYear}  |  Box ${opts.box}  |  Page ${opts.pageNum} of ${opts.totalPages}`, MARGIN, y, { align: 'center', width: CONTENT_WIDTH });
    y += 16;
    ...
    for (const col of cols) {
        doc.text(col.label, col.x, headerY, { width: col.w, align: col.x > MARGIN + 200 ? 'right' : 'left' });
    }
    y = headerY + 12;
    ...
    for (const line of opts.lines) {
        ...
        doc.text(fmt(line.proceeds), cols[3].x, y, { width: cols[3].w, align: 'right' });
        doc.text(fmt(line.costBasis), cols[4].x, y, { width: cols[4].w, align: 'right' });
        doc.text(line.adjustmentCode || '', cols[5].x, y, { width: cols[5].w, align: 'right' });
        doc.text(line.adjustmentAmount ? fmt(line.adjustmentAmount) : '', cols[6].x, y, { width: cols[6].w, align: 'right' });
        doc.text(fmt(line.gainLoss), cols[7].x, y, { width: cols[7].w, align: 'right' });
        y += 14;
    }
    ...
    if (opts.summary) {
        ...
        doc.fontSize(8).font('Helvetica-Bold')
            .text(`Totals (${opts.summary.lineCount} items)`, cols[0].x, y, { width: cols[0].w + cols[1].w + cols[2].w });
        doc.text(fmt(opts.summary.totalProceeds), cols[3].x, y, { width: cols[3].w, align: 'right' });
        doc.text(fmt(opts.summary.totalCostBasis), cols[4].x, y, { width: cols[4].w, align: 'right' });
        doc.text(fmt(opts.summary.totalAdjustments), cols[6].x, y, { width: cols[6].w, align: 'right' });
        doc.text(fmt(opts.summary.totalGainLoss), cols[7].x, y, { width: cols[7].w, align: 'right' });
    }

    // Footer
    doc.fontSize(7).font('Helvetica').fillColor('#888888')
        .text('Generated by DTax (dtax.dev) — For informational purposes only', MARGIN, 740, { align: 'center', width: CONTENT_WIDTH });
    doc.fillColor('#000000');
}

function renderScheduleDPage(doc: PDFKit.PDFDocument, scheduleD: ScheduleDReport, taxpayerName?: string): void {
    let y = MARGIN;

    doc.fontSize(14).font('Helvetica-Bold')
        .text('Schedule D (Form 1040)', MARGIN, y, { align: 'center', width: CONTENT_WIDTH });
    y += 18;

    doc.fontSize(10).font('Helvetica')
        .text('Capital Gains and Losses', MARGIN, y, { align: 'center', width: CONTENT_WIDTH });
    y += 14;

    doc.fontSize(8).font('Helvetica')
        .text(`Tax Year ${scheduleD.taxYear}`, MARGIN, y, { align: 'center', width: CONTENT_WIDTH });
    y += 10;
    ...
    const renderPart = (title: string, lines: ScheduleDReport['partI'], netLabel: string, netValue: number) => {
        ...
        for (const line of lines) {
            ...
            doc.text(line.proceeds ? fmt(line.proceeds) : '—', proceedsCol, y, { width: colW, align: 'right' });
            doc.text(line.costBasis ? fmt(line.costBasis) : '—', basisCol, y, { width: colW, align: 'right' });
            doc.text(line.gainLoss ? fmt(line.gainLoss) : '—', glCol, y, { width: colW, align: 'right' });
            y += 14;
        }
    ...
    renderPart('Part I — Short-Term Capital Gains and Losses', scheduleD.partI, 'Net Short-Term (Line 7)', scheduleD.netShortTerm);
    renderPart('Part II — Long-Term Capital Gains and Losses', scheduleD.partII, 'Net Long-Term (Line 15)', scheduleD.netLongTerm);

    // Combined summary
    doc.moveTo(MARGIN, y).lineTo(MARGIN + CONTENT_WIDTH, y).lineWidth(1).stroke();
    y += 8;

    doc.fontSize(10).font('Helvetica-Bold')
        .text('Line 16: Combined Net Gain/Loss', MARGIN, y);
    doc.text(fmt(scheduleD.combinedNetGainLoss), MARGIN + CONTENT_WIDTH - 100, y, { width: 100, align: 'right' });
    y += 18;

    if (scheduleD.capitalLossDeduction > 0) {
        doc.fontSize(9).font('Helvetica')
            .text('Line 21: Capital Loss Deduction (max $3,000)', MARGIN, y);
        doc.text(`(${fmt(scheduleD.capitalLossDeduction)})`, MARGIN + CONTENT_WIDTH - 100, y, { width: 100, align: 'right' });
        y += 14;

        if (scheduleD.carryoverLoss > 0) {
            doc.text('Loss Carryover to Next Year', MARGIN, y);
            doc.text(fmt(scheduleD.carryoverLoss), MARGIN + CONTENT_WIDTH - 100, y, { width: 100, align: 'right' });
        }
    }
