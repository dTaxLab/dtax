 ...
 *
 * When a CPA firm name is supplied, the footer reads:
 * "Prepared by [preparedBy] · Powered by dTax (getdtax.com)"
 *
 * @param doc        - The PDFKit document instance
 * @param preparedBy - Optional CPA firm name for branded footer
 */
export function renderFooter(
  doc: PDFKit.PDFDocument,
  preparedBy?: string,
): void {
  const text = preparedBy
    ? `Prepared by ${preparedBy} · Powered by dTax (getdtax.com)`
  ...
  doc
    ...
    .font("Helvetica")
    .fillColor("#888888")
    .text(text, MARGIN, PAGE_BOTTOM, { align: "center", width: CONTENT_WIDTH });
  doc.fillColor("#000000");
}

/**
 * Render a CPA firm logo in the top-left corner of the current page position.
 *
 * The logo is constrained to 150×50 points with aspect-ratio preserved.
 * Silently skips rendering if the buffer is an unsupported image format.
 *
 * @param doc            - The PDFKit document instance
 * @param y              - Current vertical position
 * @param firmLogoBuffer - Raw image buffer (JPEG or PNG)
 * @returns Updated y position after the logo (y + 58).
 */
export function renderFirmLogo(
  doc: PDFKit.PDFDocument,
  y: number,
  firmLogoBuffer: Buffer,
): number {
  try {
    doc.image(firmLogoBuffer, MARGIN, y, { fit: [150, 50] });
    return y + 58;
  } catch {
    // Invalid image data — skip silently rather than crashing the PDF
    return y;
  }
}
