...
export interface Form8949Options {
  ...
  ) => "reported_correct" | "reported_incorrect" | "not_reported";
...
export function generateForm8949(
  ...
  for (const box of ["A", "B", "C", "D", "E", "F"] as Form8949Box[]) {
    ...
    boxSummaries.push({
      ...
      totalAdjustments: round2(
        boxLines.reduce((s, l) => s + l.adjustmentAmount, 0),
      ),
  ...
  return {
    ...
    boxSummaries,
    totals: {
      shortTermGainLoss: round2(
        shortTermLines.reduce((s, l) => s + l.gainLoss, 0),
      ),
      longTermGainLoss: round2(
        longTermLines.reduce((s, l) => s + l.gainLoss, 0),
      ),
