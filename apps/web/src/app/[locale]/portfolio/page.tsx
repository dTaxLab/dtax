...
export default function PortfolioPage() {
    ...
    const [appliedPrices, setAppliedPrices] = useState<Record<string, number> | undefined>();
    ...
    if (loading && !analysis) {
        return (
            <div style={{ padding: '60px 0', textAlign: 'center' }}>
                ...
                <p style={{ color: 'var(--text-muted)', marginTop: '16px' }}>{t('loading')}</p>
            </div>
        );
    }
        ...
        <div className="animate-in">
            ...
            {analysis && (
                <div className="grid-4" style={{ marginBottom: '32px' }}>
                    <div className="stat-card">
                        ...
                        <span className="stat-value neutral">{formatUsd(analysis.totalCostBasis)}</span>
                    </div>
                    <div className="stat-card">
                    ...
                    <div className="stat-card">
                        ...
                        <span className={`stat-value ${
                            analysis.totalUnrealizedGainLoss !== undefined
                                ? analysis.totalUnrealizedGainLoss >= 0 ? 'positive' : 'negative'
                                : 'neutral'
                        }`}>
                    ...
                    <div className="stat-card">
                        ...
                        <span className="stat-value neutral">{analysis.positions.length}</span>
                    </div>
                </div>
            )}
            ...
            {analysis && analysis.positions.length === 0 ? (
                <div className="card" style={{ textAlign: 'center', padding: '48px' }}>
                    ...
                    <p style={{ color: 'var(--text-muted)', fontSize: '13px', marginTop: '8px' }}>{t('noPositionsHint')}</p>
                </div>
            ) : analysis && (
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                {hasPrices && <th style={{ textAlign: 'right' }}>{t('currentPrice')}</th>}
                                <th style={{ textAlign: 'right' }}>{t('totalCostBasis')}</th>
                                {hasPrices && <th style={{ textAlign: 'right' }}>{t('value')}</th>}
                                {hasPrices && <th style={{ textAlign: 'right' }}>{t('gainLoss')}</th>}
                        ...
                        <tbody>
                            {analysis.positions.map(pos => (
                                <tr key={pos.asset}>
                                    <td style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{pos.asset}</td>
                                    <td className="mono" style={{ textAlign: 'right' }}>
                                        {pos.totalAmount.toLocaleString('en-US', { maximumFractionDigits: 8 })}
                                    </td>
                                    <td className="mono" style={{ textAlign: 'right' }}>{formatUsd(pos.avgCostPerUnit)}</td>
                                    {hasPrices && (
                                        <td className="mono" style={{ textAlign: 'right' }}>{formatUsd(pos.currentPrice)}</td>
                                    )}
                                    <td className="mono" style={{ textAlign: 'right' }}>{formatUsd(pos.totalCostBasis)}</td>
                                    {hasPrices && (
                                        <td className="mono" style={{ textAlign: 'right' }}>{formatUsd(pos.currentValueUsd)}</td>
                                    )}
                                    {hasPrices && (
                                        <td className="mono" style={{ textAlign: 'right' }}>
                                            <span style={{
                                                color: (pos.unrealizedGainLoss ?? 0) >= 0 ? 'var(--green)' : 'var(--red)',
                                            }}>
            ...
            {analysis && (
                <div style={{ marginBottom: '32px' }}>
                    <h2 style={{ fontSize: '18px', fontWeight: '600', marginBottom: '16px' }}>{t('tlh.title')}</h2>

                    {analysis.tlhOpportunities.length === 0 ? (
                        <div className="card" style={{ textAlign: 'center', padding: '32px' }}>
                            <p style={{ color: 'var(--text-muted)' }}>
                                {hasPrices ? t('tlh.noOpportunities') : t('tlh.noOpportunitiesHint')}
                            </p>
                        </div>
                    ) : (
                        <>
                            <div className="stat-card" style={{ marginBottom: '16px', borderColor: 'var(--red)', borderWidth: '2px' }}>
                                <span className="stat-label">{t('tlh.totalAvailable')}</span>
                                <span className="stat-value negative">{formatUsd(analysis.totalTlhAvailable)}</span>
                            </div>

                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                {analysis.tlhOpportunities.map(opp => (
                                    <div key={opp.asset} className="card" style={{ padding: '16px' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                                            <span style={{ fontWeight: 600, fontSize: '16px' }}>{opp.asset}</span>
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', fontSize: '14px' }}>
                                            <div>
                                                <span style={{ color: 'var(--text-muted)', fontSize: '12px' }}>{t('tlh.amountToSell')}</span>
                                                <div className="mono">{formatAmount(opp.totalAmount, opp.asset)}</div>
                                            </div>
                                            <div>
                                                <span style={{ color: 'var(--text-muted)', fontSize: '12px' }}>{t('tlh.costBasis')}</span>
                                                <div className="mono">{formatUsd(opp.totalCostBasis)}</div>
                                            </div>
                                            <div>
                                                <span style={{ color: 'var(--text-muted)', fontSize: '12px' }}>{t('tlh.currentValue')}</span>
                                                <div className="mono">{formatUsd(opp.currentValue)}</div>
                                            </div>
                                        </div>
