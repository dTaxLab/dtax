...
// @ts-ignore
import type { DeepHealthStatus } from "../models";
// @ts-ignore
...
export const HealthApiAxiosParamCreator = function (
  ...
  return {
     ...
     * @throws {RequiredError}
     */
    healthCheckDeep: async (
      options: RawAxiosRequestConfig = {},
    ): Promise<RequestArgs> => {
      const localVarPath = `/api/health/deep`;
      // use dummy base URL string because the URL constructor only accepts absolute URLs.
      const localVarUrlObj = new URL(localVarPath, DUMMY_BASE_URL);
      let baseOptions;
      if (configuration) {
...
export const HealthApiFp = function (configuration?: Configuration) {
  ...
  return {
     ...
     * @throws {RequiredError}
     */
    async healthCheckDeep(
      options?: RawAxiosRequestConfig,
    ): Promise<
      (
        axios?: AxiosInstance,
        basePath?: string,
      ) => AxiosPromise<DeepHealthStatus>
    > {
      const localVarAxiosArgs =
        await localVarAxiosParamCreator.healthCheckDeep(options);
      const localVarOperationServerIndex = configuration?.serverIndex ?? 0;
      const localVarOperationServerBasePath =
        operationServerMap["HealthApi.healthCheckDeep"]?.[
          localVarOperationServerIndex
        ]?.url;
      return (axios, basePath) =>
        createRequestFunction(
...
export class HealthApi extends BaseAPI implements HealthApiInterface {
   ...
   * @throws {RequiredError}
   */
  public healthCheckDeep(options?: RawAxiosRequestConfig) {
    return HealthApiFp(this.configuration)
      .healthCheckDeep(options)
      .then((request) => request(this.axios, this.basePath));
  }
